...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['uname'], $_POST['pswd'], $_POST['defPath'])) {
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "INSERT INTO users (uname, pswd, admin, defPath) VALUES (?, ?, ?, ?)";
            echo "<script>console.log('SQL: " . $sql . "');</script>";
            echo "<script>console.log('Params: " . json_encode([$uname, $pswd, $is_admin, $defPath]) . "');</script>";

            $stmt = $conn->prepare($sql);
            $hash = password_hash($pswd, PASSWORD_BCRYPT);
            $stmt->bind_param("ssis", $uname, $hash, $is_admin, $defPath);
...
$result = $conn->query("SELECT uname, admin, defPath FROM users");
?>
<!DOCTYPE html>
<html>
<head>
    ...
    <section class="row">
        ...
        <article class="col border border-2 border-primary rounded p-2">
            ...
            <form method="POST" action="" class="mb-4 add-user-form">
                ...
                <div class="form-check mb-3 center">
